// Bithereum (BTH) block header has differences compared to Bitcoin block headers
// https://github.com/BTHPOS/BTHPOS/wiki/Technical-Spec
...
BlockBeth.prototype = Object.create(Block.prototype)

BlockBeth.fromBuffer = function (buffer) {
...
BlockBeth.prototype.byteLength = function (headersOnly, useLegacyFormat) {
  var headerSize = 0
  if (useLegacyFormat) {
    headerSize = 80
  } else {
...
BlockBeth.fromHex = function (hex) {
  return BlockBeth.fromBuffer(Buffer.from(hex, 'hex'))
}

BlockBeth.prototype.getHash = function (network) {
  network = network || networks.bithereum
...
BlockBeth.prototype.toBuffer = function (headersOnly, useLegacyFormat) {
...
BlockBeth.prototype.toHex = function (headersOnly, useLegacyFormat) {
  return this.toBuffer(headersOnly, useLegacyFormat).toString('hex')
}

BlockBeth.prototype.checkProofOfWork = function (validateSolution, network, previousBlocks) {
  network = network || networks.bithereum
  ...
  if (validateSolution && this.height >= network.forkHeight) {
    ...
    var equihashParams = network.equihash || eq.networks.bithereum
